...
def evolve_lagrangian_front(points, Tcur, x_coords, z_coords, v_mig, dt_s, dist_reprod):
    ...
    for _ in range(num_substeps):
        ...
        # Evaluate reproduction into empty spaces.
        if all_cand.size > 0:
            angles = np.random.rand(all_cand.shape[0]) * 2 * np.pi
            test_offsets = np.column_stack((np.cos(angles), np.sin(angles))) * dist_reprod
            test_points = all_cand + test_offsets
            
            test_points[:, 0] = np.clip(test_points[:, 0], 0, Lx)
            test_points[:, 1] = np.clip(test_points[:, 1], 0, Lz)
            c_temps_test = rgi(np.column_stack((test_points[:, 1], test_points[:, 0])))
            
            valid_mask = (c_temps_test >= Tmin_life) & (c_temps_test <= 100.0)
            
            if np.any(valid_mask):
                valid_parents = all_cand[valid_mask]
                valid_test_points = test_points[valid_mask]
                
                tree_existing = cKDTree(all_cand)
                dists_to_existing, _ = tree_existing.query(valid_test_points, k=1)
                
                empty_mask = dists_to_existing > marker_spacing
