...
def evolve_lagrangian_front(points, Tcur, x_coords, z_coords, v_mig, dt_s, dist_reprod):
    ...
    for _ in range(num_substeps):
        ...
        # Evaluate reproduction into empty spaces. Children are cast outward at dist_reprod.
        if all_cand.size > 0:
            angles = np.random.rand(all_cand.shape[0]) * 2 * np.pi
            offsets = np.column_stack((np.cos(angles), np.sin(angles))) * dist_reprod
